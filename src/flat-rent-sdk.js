...
export class FlatRentSdk {
   ...
   */
  search(parameters) {
    parameters.checkIn = new Date(parameters.checkIn)
    parameters.checkOut = new Date(parameters.checkOut)
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!(parameters.checkIn instanceof Date) || !(parameters.checkOut instanceof Date)) {
          throw new Error(`Passed invalid check-in or check-out date - from "${parameters.checkIn}" to "${parameters.checkOut}".`)
        }
        this._assertDatesAreCorrect(parameters.checkIn, parameters.checkOut)
        ...
        const dateRange = this._generateDateRange(parameters.checkIn, parameters.checkOut)
        flats = flats.filter((flat) => {
  ...
  book(flatId, checkIn, checkOut) {
    return new Promise((resolve, reject) => {
      try {
        ...
        this._assertDatesAreCorrect(checkIn, checkOut)

        const datesToBook = this._generateDateRange(checkIn, checkOut)
  ...
  _assertDatesAreCorrect(checkIn, checkOut) {
    const today = new Date()
    this._resetTime(today)
    this._resetTime(checkIn)
    this._resetTime(checkOut)

    const diffToday = this._calculateDifferenceInDays(today, checkIn)
    if (diffToday < 0) {
      throw new Error('Check-in date can\'t be in the past.')
    }

    const diffCheck = this._calculateDifferenceInDays(checkIn, checkOut)
    if (diffCheck < 0) {
  ...
  _formatFlatObject(flat, nightNumber) {
    ...
    formattedFlat.img = formattedFlat.photos.map((photoUrl) => {
